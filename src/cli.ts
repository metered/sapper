...
prog.command('dev')
	...
	.option('--bundler_config', 'Path to rollup.config.js or webpack.config.js relative to the cwd.')
	...
	.action(async (opts: {
		...
		try {
			const watcher = dev({
				...
				bundler_config: opts.bundler_config,
				ext: opts.ext
			});
...
prog.command('build [dest]')
	...
	.action(async (dest = '__sapper__/build', opts: {
		...
		try {
			await _build(opts.bundler, opts.legacy, opts.cwd, opts.src, opts.routes, opts.output, dest, opts.ext, opts.static, opts.dev, opts.bundler_config);
...
prog.command('export [dest]')
	...
	.action(async (dest = '__sapper__/export', opts: {
		...
		try {
			if (opts.build) {
				console.log(`> Building...`);
				await _build(opts.bundler, opts.legacy, opts.cwd, opts.src, opts.routes, opts.output, opts['build-dir'], opts.ext, opts.static, false, opts.bundler_config);
