...
import { Page, PagePart, DefaultPageComponent, PageComponent, ServerRoute, ManifestData, UserPageComponent } from '../interfaces';
	...
	function walk(
		...
		items.forEach(item => {
			...
			if (item.is_dir) {
				...
				const parts: PagePart[] = ((item.is_index && stack[stack.length - 1] === null)
					? stack.slice(0, -1).concat({ component, params })
					: stack.concat({ component, params })) as PagePart[]
